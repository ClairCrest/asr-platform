...
export function TranscriptViewer({ transcript }: { transcript: Transcript }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="max-h-96 space-y-1 overflow-y-auto rounded-lg border border-gray-200 bg-white p-4">
        {transcript.segments.length === 0 && <p className="text-sm text-gray-400">{transcript.text}</p>}
        {transcript.segments.map((segment) => (
          <button
            ...
            className={`block w-full rounded-md px-2 py-1.5 text-left text-sm ${
              activeIdx === segment.idx ? 'bg-gray-100' : 'hover:bg-gray-50'
            }`}
          >
            <span className="mr-2 font-mono text-xs text-gray-400">{formatMs(segment.start_ms)}</span>
